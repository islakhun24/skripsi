...
export default function (props) {
  ...
  const shareText = React.useMemo(
    () => `Gabung ke kelas online ${eventSessionDetails.title} di Vico UMP. `,
    [eventSessionDetails.title]
  );

  const link = React.useMemo(
    () => getUrl() + routes.EVENT_SESSION(sessionId),
    [sessionId]
  );
        ...
        <div className={classes.content}>
          ...
          >
            ...
            <Tooltip title="Salin link kelas">
              <CopyToClipboard
                text={link}
                onCopy={() => snackbar.showMessage("link kelas disalin")}
              >
                <IconButton
                  aria-label="copy"
                  style={{ marginLeft: 8 }}
          ...
          {eventSessionDetails && (
            <EventShareIcons
              url={
                getUrl() +
                routes.EVENT_SESSION(eventSessionDetails.originalSessionId)
              }
              shareText={shareText}
            />
          )}
