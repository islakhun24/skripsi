...
const EventSessionContainer = React.memo(() => {
  ...
  const isLive = React.useMemo(() => {
    ...
    const { eventBeginDate, eventOpens, eventEndDate, eventCloses } =
      eventSessionDetails;
  ...
  const smallPlayerBounds = React.useMemo(
    () => `.${classes.root}`,
    [classes.root]
  );
  ...
  if (!eventSessionDetails) {
    return (
      <Page title="Vico UMP | Event not found">
        <div
          className={clsx({
            [classes.root]: true,
